/*
	Curious incident Awards Banner
	==============================

	Todo
	----------------------------------------------------------------------------------------------------
	- Click actions
	  Can probably be done by using a click action on the cr_banner div
	- Looping
	  This can probably be done by calling firstFrame() and initialize() functions
	  Also setting doRender and window.anim_render to true
	  Use the loopCount variable to achieve this - Much like you would in a flash banner

	Notes
	----------------------------------------------------------------------------------------------------
	Both this file and the css files should be minified. This will shave off a few K
...
(function() {
	...
	*/
	...
	$( document ).ready(function() {
		...
		// This is the banner text array (particles) - Add a new line for each one.

		var str1 = [
			"<span class='first-line'>WINNER</span><br><span class='second-line'>5 Tony Awards<sup>&copy;</span>",
			"<span class='first-line'>WINNER</span><br><span class='second-line'>7 Olivier Awards</span>"
		];

		// Loop counter (not implemented at present)

		var loopCount = 0;

		// ========================================================
		// DO THE PARTICLE STORM
		// ========================================================
		...
		window.anim_render = true; // Set this as a global so we can refer to it later

		var canvas = document.getElementById('particleCanvas'), // Create the particle canvas reference
			context,
			particles = [];
			...
			if (canvas && canvas.getContext) {
				...
				for (var i=0; i < particlesQuantity; i++) {
					...
					// Set up each particle position (initial)
					particles.push({
						...
						color: 'rgba(255,255,255,0.6)' // Set the particle color
					});
				}

			initialize(); // Animate them
		}
		...
		function renderFrame() { // Animation engine
		...
		function initialize() { // Do the animation loop
			window.addEventListener('resize',resizeCanvas,false);
			if (window.anim_render) {
		...
		// ========================================================
		// DO THE PARTICLE TEXT ANIMATION
		// ========================================================
		...
		var ctx1;
		var speed = 4; // Speed
		var density = 1; // Pixel Density
		var counter = 0; // Count
		var animKeyframe;
		var doRender = true;

		var theStage = document.getElementById('c');
		...
		function render() { // Does the dot positioning
			if (doRender) {
				requestAnimationFrame(render);

				var ctx = c.getContext('2d');
				ctx.clearRect(0, 0, theStage.width*2, theStage.height)*2;

				for (var i = 0; i < dots.length; i++) {

					var dx = Math.abs(dots[i].x - dots[i].X) / speed;
					var dy = Math.abs(dots[i].y - dots[i].Y) / speed;
					var dr = Math.abs(dots[i].r - dots[i].R) / speed;
					var da = Math.abs(dots[i].a - dots[i].A) / speed;
					...
					if (dots[i].x < dots[i].X) {
						dots[i].x += dx;
					} else {
						dots[i].x -= dx;
					}

					if (dots[i].y < dots[i].Y) {
						dots[i].y += dy;
					} else {
						dots[i].y -= dy;
					}

					if (dots[i].r < dots[i].R) {
						dots[i].r += dr;
					} else {
						dots[i].r -= Math.round(dr);
					}

					if (dots[i].a < dots[i].A) {
						dots[i].a += da;
					} else {
						dots[i].a -= da;
					}
		...
		function run() { // Map each dot to a position
			...
			// Translate the HTML and convert to an image then build a pixel matrix for the particles to stick to (eek)
			...
			html2canvas(document.getElementById('container'), {
				...
				onrendered: function (canvas) {
					...
					var img = ctx1.getImageData(0, 0, canvas.width, canvas.height);
					var imgData = img.data;

					var stageWidth = canvas.width;
					var stageHeight = canvas.height;

					for (var j = 0; j < stageHeight; j ++) { // This is where the magic happens
						for (var i = 0; i < stageWidth; i ++) {
							var numItems = (j * stageWidth + i) * 4;
			...
			if (counter === 2) {
				clearInterval(animKeyframe);
				finalFrames() // Callback that sets off the last frame
			}
		}

		// ========================================================
		// SORT OF A TIMELINE FUDGED HACK OF DOOM
		// ========================================================

		// All these animations could be done with Greensock - But only if you need easing

		// Frame 1 (animates in the NT Logo)

		function firstFrame () { // First frame
		    $('.nt-logo').animate({
		    	"opacity":1
			}, 1000, function() {
				doStrapLine(); // Callback
			});
		}

		// --- Frame 1 strapline

		function doStrapLine() { // Second frame
			$('.sywd').animate({
				"opacity":1
			}, 1000, function() { // Animates the first frame text after a short delay
				$('.sywd').delay(2000).animate({
					"opacity":0
				}, 1000, function() { // Set off the particle words.
					thirdFrame();
				})
			});
		}

		// --- Frame 2 & 3 (particle text)

		function thirdFrame() {
			render();
			run();
			window.anim_render = false; // Shut down the rendering or it causes a memory leak in Chrome
			animKeyframe = setInterval(function() {
				run();
			}, 6000);
		}
		...
		function finalFrames() {
			$('#c').delay(3000).animate({ // Lose the text particle canvas (After a short delay)
				"opacity":0
			}, 1000, function() {
				fadeInBooking();
				doRender = false; // Shut down the rendering or it causes a memory leak in Chrome
			});
		}

		function fadeInBooking() { // Last frame
			$('#particleCanvas').animate({ // Lose the particle canvas
				"opacity":0
			}, 1000);
			$('.last-frame ').animate({ // Fade up the last frame
				"opacity":1
			}, 1000);
		...
		firstFrame();  // START THE WHOLE ANIMATION
